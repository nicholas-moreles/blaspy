...
def test_daxpy():
    ...
    y2 = np.copy(y1)
    daxpy(n, alpha, np.ctypeslib.as_ctypes(x1), 1, np.ctypeslib.as_ctypes(y1), 1, Vec.ROW_ROW)
    y2 += alpha * x1
    ...
    y2 = np.copy(y1)
    daxpy(n, alpha, np.ctypeslib.as_ctypes(x1), 1, np.ctypeslib.as_ctypes(y1), 1, Vec.COL_COL)
    y2 += alpha * x1
    passed = np.allclose(y1, y2)
    if not passed: tests_failed.append(test_num)
