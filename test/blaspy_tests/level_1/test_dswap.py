...
def test_dswap():
    ...
    x2 = np.copy(x1)
    y2 = np.copy(y1)
    dswap(n, np.ctypeslib.as_ctypes(x2), 1, np.ctypeslib.as_ctypes(y2), 1, Vec.ROW_ROW)
    ...
    x2 = np.copy(x1)
    y2 = np.copy(y1)
    dswap(n, np.ctypeslib.as_ctypes(x2), 1, np.ctypeslib.as_ctypes(y2), 1, Vec.ROW_COL)
    ...
    x2 = np.copy(x1)
    y2 = np.copy(y1)
    dswap(n, np.ctypeslib.as_ctypes(x2), 1, np.ctypeslib.as_ctypes(y2), 1, Vec.COL_ROW)
    passed = np.allclose(np.transpose(x1), y2) and np.allclose(np.transpose(x2), y1)
    if not passed: tests_failed.append(test_num)
    ...
    x2 = np.copy(x1)
    y2 = np.copy(y1)
    dswap(n, np.ctypeslib.as_ctypes(x2), 1, np.ctypeslib.as_ctypes(y2), 1, Vec.COL_COL)
    passed = np.allclose(x1, y2) and np.allclose(x2, y1)
    if not passed: tests_failed.append(test_num)
